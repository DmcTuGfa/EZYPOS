...
## 12. Porciones y extras en productos (puestos de comida / venta a granel)

### En Productos
El formulario de alta/edición tiene dos secciones nuevas:
- **Porciones de venta** — para vender por 1/4, 1/2, 1 kg, etc. Cada porción lleva etiqueta, cantidad que descuenta del inventario (1/2 kg = 0.5 en la unidad del producto) y su precio. Si el producto tiene porciones, el precio de venta normal deja de usarse en el POS.
- **Extras** — complementos con costo adicional (cueritos, carne, pata, buche...). Precio 0 = sin costo.

### En el Punto de Venta
- Los productos con porciones o extras muestran una insignia "Opciones" y su precio como "Desde $X".
- Al tocarlos se abre un selector: porción (botones grandes), extras (casillas con su precio) y cantidad, con el total calculado en vivo. Aplica también al buscador.
- El mismo producto con distinta porción o extras ocupa **líneas separadas** en el carrito (2 × 1/2 kg con cueritos y 1 × 1/4 sin extras conviven sin mezclarse).
- El escáner de código de barras agrega la primera porción sin extras y avisa para cambiarla desde el carrito si es otra.

### Inventario y ticket
- Vender 1/2 kg descuenta **0.5** de la existencia (no 1). El precio unitario se ajusta para que cantidad × precio = total cobrado.
- En el ticket y el historial la línea sale descriptiva: "Carnitas (1/2 kg) + cueritos, pata".
- El reporte de ingresos y egresos refleja las cantidades reales en la unidad del producto.

### Base de datos
Columnas `portions` y `extras` (JSONB) en `products`, agregadas con `ADD COLUMN IF NOT EXISTS` — se crean solas al arrancar, sin migración manual ni riesgo para los productos existentes.

**Archivos:** `lib/server/setup.ts`, `lib/types/index.ts`, `lib/server/mappers.ts`, `app/api/products/route.ts`, `app/api/products/[id]/route.ts`, `app/(auth)/products/page.tsx`, `lib/stores/cart-store.ts`, `lib/stores/sales-store.ts`, `components/pos/product-options-modal.tsx` (nuevo), `components/pos/product-grid.tsx`, `components/pos/product-search.tsx`, `components/pos/cart-scan-button.tsx`, `components/pos/cart-list.tsx`
